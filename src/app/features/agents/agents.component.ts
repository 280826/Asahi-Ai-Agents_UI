...
export class AgentsComponent {
  ...
  private normalizeProcessLabel(a: Agent): string {
    // Try several fields to resolve the process name robustly
    const candidates = [
      a?.industry,
      (a as any)?.process,
      (a as any)?.category,
      (a as any)?.domain,
      (a as any)?.valueStream,
    ]
      .map((v) => (typeof v === 'string' ? v.trim() : ''))
      .filter(Boolean);

    // Prefer the first non-empty; allow simple aliases
    const raw = (candidates[0] || '').toLowerCase();

    if (raw.includes('procure') || raw.includes('p2p')) return 'Procure to Pay';
    if (raw.includes('order') || raw.includes('o2c')) return 'Order to Cash';
    if (raw.includes('record') || raw.includes('r2r') || raw.includes('report'))
      return 'Record to Report';

    // Fallback: if industry is something else, return that verbatim
    // (won’t count towards the 3 buckets unless it matches an alias above)
    return (candidates[0] || '').trim();
  }

  /** Computed metrics for the three buckets */
  readonly metrics = computed(() => {
    ...
    for (const a of list) {
      const p = this.normalizeProcessLabel(a);
      if (p === 'Procure to Pay') p2p++;
      else if (p === 'Order to Cash') otc++;
      else if (p === 'Record to Report') rtr++;
    }
